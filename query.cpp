...
int main(int argc, char *argv[]){
    ...
    std::vector<std::string> str_sa{"out_11k_0", "out_11k_3", "out_11k_6", "out_11k_9", "out_11k_12", "out_11k_15", "out_20k_0", "out_20k_3", "out_20k_6", "out_20k_9", "out_20k_12", "out_20k_15", "out_1.6M_0", "out_1.6M_3", "out_1.6M_6", "out_1.6M_9", "out_1.6M_12", "out_1.6M_15", "out_4.6M_0", "out_4.6M_3", "out_4.6M_6", "out_4.6M_9", "out_4.6M_12", "out_4.6M_15", "out_49M_0", "out_49M_3", "out_49M_6", "out_49M_9", "out_49M_12", "out_49M_15"};

    for( uint64_t idx = 0 ; idx < str_sa.size(); idx++){
        std::cout<<"idx "<<idx<<"\n";
    //Default Parameters
    std::string address_SA = str_sa[idx];
    ...
    std::ofstream fs1("naive_search1_short.txt", std::ios::app);
    std::ofstream fs2("naive_search1_long.txt", std::ios::app);
    std::ofstream fs3("naive_search2_short.txt", std::ios::app);
    std::ofstream fs4("naive_search2_long.txt", std::ios::app);
    std::ofstream fs5("simpaccel_search1_short.txt", std::ios::app);
    std::ofstream fs6("simpaccel_search1_long.txt", std::ios::app);
    std::ofstream fs7("simpaccel_search2_short.txt", std::ios::app);
    std::ofstream fs8("simpaccel_search2_long.txt", std::ios::app);
    fs1 << ">>" << str_sa[idx] << "\n";
    fs2 << ">>" << str_sa[idx] << "\n";
    fs3 << ">>" << str_sa[idx] << "\n";
    fs4 << ">>" << str_sa[idx] << "\n";
    ...
    //std::fstream fs("naive_search1_short.txt", std::ios::in | std::ios::out | std::ios::ate);
     ...
     if(query_mode == "naive" && method == "1" && range == "short"){
        ...
        {   
            ...
            naive_search1_short(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            auto stop = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> elapsedTime = (stop - start);
            fs1 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            naive_search1_long(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs2 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            naive_search2_short(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs3 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            naive_search2_long(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs4 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            simpaccel_search1_short(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs5 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            simpaccel_search1_long(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs6 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            simpaccel_search2_short(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs7 << elapsedTime.count()<<"\n";

            start = std::chrono::high_resolution_clock::now();
            simpaccel_search1_long(sequence, SA, pref_string, pref_first, pref_last,query_sequences[i]);
            stop = std::chrono::high_resolution_clock::now();
            elapsedTime = (stop - start);
            fs8 << elapsedTime.count()<<"\n";
